...
describe('Smoke: devices', () => {
  ...
  for (const exp of EXPECTED_DEVICES) {
    it(`device ${exp.ia} — ${exp.name.substring(0, 30)}`, () => {
      ...
      assert.equal((d.parameters || []).length, exp.paramCount, `param count for ${exp.ia}`);
    });
  }
...
describe('Smoke: group addresses', () => {
  ...
  const EXPECTED_GAS = [
    { address: '1/0/0', name: 'Chandelier On/Off', dpt: 'DPST-1-1', mainGroupName: 'Lighting', middleGroupName: 'Kitchen' },
    { address: '2/0/0', name: 'Blind Up/Down', dpt: '', mainGroupName: 'Blinds', middleGroupName: 'Kitchen' },
    { address: '11/0/0', name: 'Chandelier Status', dpt: 'DPST-1-1', mainGroupName: 'Lighting Status', middleGroupName: 'Kitchen' },
    { address: '12/0/0', name: 'Blind Percentage', dpt: '', mainGroupName: 'Blinds Status', middleGroupName: 'Kitchen' },
  ];
...
describe('Smoke: communication objects', () => {
  ...
  it('LED dimmer channel A switching is linked to Chandelier On/Off and Status', () => {
    const co = parsed.comObjects.find(co => co.device_address === '1.1.3' && co.object_number === 7);
    assert(co, 'dimmer channel A switching CO not found');
    assert.equal(co.name, 'Channel A: Switching');
    assert(co.ga_address.includes('1/0/0'), 'should be linked to Chandelier On/Off');
...
describe('Smoke: API import', () => {
  ...
  it('devices in database have correct manufacturer from knx_master.xml', () => {
    ...
    for (const d of devs) {
      assert.equal(d.manufacturer, 'ABB AG - STOTZ-KONTAKT', `${d.individual_address} manufacturer`);
    }
  });
  ...
  it('ga_group_names has main and middle group names', () => {
    const names = db.all('SELECT * FROM ga_group_names WHERE project_id=?', [pid]);
  ...
  it('cleanup — delete project', async () => {
    ...
    assert.equal(db.get('SELECT count(*) as c FROM devices WHERE project_id=?', [pid]).c, 0);
    assert.equal(db.get('SELECT count(*) as c FROM topology WHERE project_id=?', [pid]).c, 0);
