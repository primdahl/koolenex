...
import { Btn, Chip, Spinner, TH, TD, SearchBox, SectionHeader, Empty, ConfirmModal, PinAddr } from '../primitives.jsx';
...
export function GroupAddressesView({ data, busConnected, activeProjectId, onWrite, onDeviceJump, onPin, onCreateGA, onDeleteGA, onUpdateGA, onRenameGAGroup, jumpTo }) {
  ...
  const [viewMode, setViewMode] = useState(() => localStorage.getItem('knx-ga-mode') || 'tree');
  useEffect(() => { localStorage.setItem('knx-ga-mode', viewMode); }, [viewMode]);
  const [expand, setExpand] = useState(() => { try { return JSON.parse(localStorage.getItem('knx-ga-expand') || '{}'); } catch { return {}; } });
  ...
  const exportGACSV = () => dlCSV('koolenex-group-addresses.csv', gaCols, filtered,
    (id, g) => ({
      address: g.address, name: g.name, dpt: g.dpt,
  ...
  useEffect(() => {
    ...
    for (const g of gas) {
      ...
      newExpand[`${g.main}/${g.middle}`] = g.main === main && middle !== null && g.middle === middle;
    }
    setViewMode('tree');
    setSearch('');
    setExpand(newExpand);
  ...
  const handleCreate = async () => {
    ...
    setNewSaving(true);
    try {
      await onCreateGA({ address: newAddr.trim(), name: newName.trim(), dpt: newDpt.trim() });
      setNewAddr(''); setNewName(''); setNewDpt(''); setCreating(false);
  ...
  const nextAddrForMid = (main, mid) => {
    const subs = gas.filter(g => g.main === main && g.middle === mid && g.sub != null).map(g => g.sub);
  ...
  const openInlineCreate = (e, main, mid = null) => {
    e.stopPropagation();
    const addr = mid === null ? nextAddrForMain(main) : nextAddrForMid(main, mid);
  ...
  const handleInlineCreate = async () => {
    ...
    try {
      await onCreateGA({ address: inlineAddr.trim(), name: inlineName.trim(), dpt: inlineDpt.trim() });
  ...
  const filtered = gas.filter(g => {
    ...
    return !s || g.address.includes(s) || g.name.toLowerCase().includes(s) || g.dpt.toLowerCase().includes(s);
  });
  const mains = [...new Set(filtered.map(g => g.main))].sort((a, b) => a - b);

  const toggleMain = (m) => setExpand(p => ({ ...p, [m]: !(p[m] ?? true) }));
  const toggleMid  = (m, mi) => setExpand(p => ({ ...p, [`${m}/${mi}`]: !(p[`${m}/${mi}`] ?? false) }));
  ...
  const GARow = ({ g, indent = 0 }) => {
    ...
    return (
      <tr className="rh"
        ...
        {gcv('address') && <TD style={{ paddingLeft: 12 + indent }}>
          <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}>
            <PinAddr address={g.address} wtype="ga" style={{ color: C.purple, fontFamily: 'monospace' }} />
            {onDeleteGA && (
              <span onClick={e => { e.stopPropagation(); setDeleteConfirm(g); }}
                title="Delete GA" style={{ position: 'absolute', right: -16, color: C.red, fontSize: 11, cursor: 'pointer', opacity: hovered ? 0.7 : 0, lineHeight: 1, transition: 'opacity 0.1s' }}>✕</span>
            )}
          </div>
        </TD>}
        ...
        {gcv('middle_group') && <TD><span style={{ color: C.dim, fontSize: 10 }}>{g.middle}{g.middle_group_name ? ` — ${g.middle_group_name}` : ''}</span></TD>}
        {gcv('description') && <TD><span style={{ color: C.dim, fontSize: 10 }}>{g.description || ''}</span></TD>}
        {gcv('comment') && <TD><span style={{ color: C.dim, fontSize: 10 }}><RtfText value={g.comment} /></span></TD>}
      </tr>
    );
  };

  const gaVisColCount = gaCols.filter(c => c.visible !== false).length;
  ...
  const assocDevices = deleteConfirm ? (gaDeviceMap[deleteConfirm.address] || []) : [];
      ...
      {deleteConfirm && (
        ...
        >
          {assocDevices.length > 0 ? (
            <>
              <div style={{ marginBottom: 8 }}>This will also remove <strong>{assocDevices.length}</strong> device association{assocDevices.length !== 1 ? 's' : ''}:</div>
              <ul style={{ margin: 0, paddingLeft: 18, maxHeight: 120, overflowY: 'auto' }}>
                {assocDevices.map(addr => {
                  const dev = devices.find(d => d.individual_address === addr);
                  return <li key={addr} style={{ fontFamily: 'monospace' }}>{addr}{dev?.name ? ` — ${dev.name}` : ''}</li>;
                })}
              </ul>
            </>
          ) : (
            <div>This group address has no device associations. It will be permanently deleted.</div>
          )}
        </ConfirmModal>
      )}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <SectionHeader title="Group Addresses" count={filtered.length} actions={[
          <SearchBox key="s" value={search} onChange={setSearch} placeholder="Search GAs…" />,
          <Chip key="tree" active={viewMode === 'tree'} onClick={() => setViewMode('tree')}>Tree</Chip>,
          <Chip key="flat" active={viewMode === 'flat'} onClick={() => setViewMode('flat')}>Flat</Chip>,
          onCreateGA && <Btn key="new" onClick={() => setCreating(p => !p)} color={C.green}>+ New GA</Btn>,
          <ColumnPicker key="cp" cols={gaCols} onChange={saveGaCols} C={C} />,
          <Btn key="csv" onClick={exportGACSV} color={C.muted} bg={C.surface}>↓ CSV</Btn>,
        ]} />
        {creating && (
          <div style={{ padding: '10px 16px', borderBottom: `1px solid ${C.border}`, background: C.surface, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <input value={newAddr} onChange={e => setNewAddr(e.target.value)} placeholder="1/2/3"
              style={{ background: C.inputBg, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '6px 10px', color: C.text, fontSize: 11, fontFamily: 'monospace', width: 80 }} />
            <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Name"
              style={{ background: C.inputBg, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '6px 10px', color: C.text, fontSize: 11, fontFamily: 'inherit', flex: 1, minWidth: 120 }} />
            <input value={newDpt} onChange={e => setNewDpt(e.target.value)} placeholder="DPT (optional)"
              style={{ background: C.inputBg, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '6px 10px', color: C.text, fontSize: 11, fontFamily: 'monospace', width: 120 }} />
            <Btn onClick={handleCreate} disabled={newSaving || !newAddr.trim() || !newName.trim()} color={C.green}>{newSaving ? <Spinner /> : 'Create'}</Btn>
            <Btn onClick={() => setCreating(false)} color={C.dim}>Cancel</Btn>
          </div>
        )}
        <div style={{ overflow: 'auto', flex: 1 }}>
          {viewMode === 'flat' ? (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr>
                {gaCols.filter(c => c.visible !== false).map(col => (
                  <TH key={col.id} style={col.id === 'address' ? { width: 100 } : col.id === 'devices' ? { width: 70 } : col.id === 'dpt' ? { width: 180 } : {}}>
                    {col.label.toUpperCase().replace('GAS','GAs')}
                  </TH>
                ))}
              </tr></thead>
              <tbody>{filtered.map(g => <GARow key={g.id} g={g} />)}</tbody>
            </table>
          ) : mains.map(main => {
            const mainGAs = filtered.filter(g => g.main === main);
            const middles = [...new Set(mainGAs.map(g => g.middle))].sort((a, b) => a - b);
            ...
            return (
              <div key={`m${main}`}>
                ...
                {inlineCreate?.main === main && inlineCreate.mid === null && (
                  <div style={{ padding: '8px 14px 8px 28px', borderBottom: `1px solid ${C.border}`, background: C.surface, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                    <input value={inlineAddr} onChange={e => setInlineAddr(e.target.value)} placeholder="m/mi"
                      style={{ background: C.inputBg, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '6px 10px', color: C.text, fontSize: 11, fontFamily: 'monospace', width: 80 }} />
                    <input value={inlineName} onChange={e => setInlineName(e.target.value)} placeholder="Group name" autoFocus
                      style={{ background: C.inputBg, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '6px 10px', color: C.text, fontSize: 11, fontFamily: 'inherit', flex: 1, minWidth: 120 }} />
                    <Btn onClick={handleInlineCreate} disabled={inlineSaving || !inlineAddr.trim() || !inlineName.trim()} color={C.green}>{inlineSaving ? <Spinner /> : 'Create'}</Btn>
                    <Btn onClick={() => setInlineCreate(null)} color={C.dim}>Cancel</Btn>
                  </div>
                )}
                {mainExpanded && middles.map(mid => {
                  const allInGroup = filtered.filter(g => g.main === main && g.middle === mid);
                  const placeholder2 = allInGroup.find(g => g.sub === null);
                  const subs = allInGroup.filter(g => g.sub !== null);
                  const midKey = `${main}/${mid}`;
                  const midExpanded = search ? true : expand[midKey] === true;
                  const midName = allInGroup.find(g => g.middle_group_name)?.middle_group_name
                    || placeholder2?.name || undefined;
                  return (
                    <div key={`m${main}mi${mid}`}>
                      ...
                      {inlineCreate?.main === main && inlineCreate.mid === mid && (
                        <div style={{ padding: '8px 14px 8px 42px', borderBottom: `1px solid ${C.border}`, background: C.hover, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                          <input value={inlineAddr} onChange={e => setInlineAddr(e.target.value)} placeholder="m/mi/s"
                            style={{ background: C.inputBg, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '6px 10px', color: C.text, fontSize: 11, fontFamily: 'monospace', width: 80 }} />
                          <input value={inlineName} onChange={e => setInlineName(e.target.value)} placeholder="Name" autoFocus
                            style={{ background: C.inputBg, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '6px 10px', color: C.text, fontSize: 11, fontFamily: 'inherit', flex: 1, minWidth: 120 }} />
                          <input value={inlineDpt} onChange={e => setInlineDpt(e.target.value)} placeholder="DPT (optional)"
                            style={{ background: C.inputBg, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '6px 10px', color: C.text, fontSize: 11, fontFamily: 'monospace', width: 120 }} />
                          <Btn onClick={handleInlineCreate} disabled={inlineSaving || !inlineAddr.trim() || !inlineName.trim()} color={C.green}>{inlineSaving ? <Spinner /> : 'Create'}</Btn>
                          <Btn onClick={() => setInlineCreate(null)} color={C.dim}>Cancel</Btn>
                        </div>
                      ...
                      {midExpanded && (
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                          <thead><tr>
                            {gaCols.filter(c => c.visible !== false).map(col => (
                              <TH key={col.id} style={
                                col.id === 'address' ? { width: 100, paddingLeft: 42 } :
                                col.id === 'devices' ? { width: 70 } :
                                col.id === 'dpt' ? { width: 180 } : {}
                              }>{col.label.toUpperCase().replace('GAS','GAs')}</TH>
                            ))}
                          </tr></thead>
                          <tbody>{subs.map(g => <GARow key={g.id} g={g} indent={30} />)}</tbody>
                        </table>
                      )}
                    </div>
                  );
