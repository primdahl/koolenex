...
import { Badge, Btn, TH, TD, SectionHeader, Empty, PinAddr, SpacePath } from '../primitives.jsx';
...
export function TopologyView({ data, onPin, busConnected, dispatch, onAddDevice, activeProjectId, onUpdateTopology, onCreateTopology, onDeleteTopology }) {
  ...
  const exportTopoCSV = () => {
    ...
    dlCSV('koolenex-topology.csv', topoCols, filtered,
      (id, d) => ({
        individual_address: d.individual_address, name: d.name,
        device_type: d.device_type, location: spacePath(d.space_id),
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 14px 5px 28px', background: C.hover, borderBottom: `1px solid ${C.border}` }}>
                      ...
                      {busConnected && dispatch && (
                        <span onClick={e => { e.stopPropagation(); dispatch({ type: 'SCAN_RESET' }); dispatch({ type: 'SET_VIEW', view: 'scan' }); api.busScan(area, line, 200); }}
                          style={{ fontSize: 9, padding: '1px 7px', borderRadius: 10, background: `${C.accent}15`, color: C.accent, border: `1px solid ${C.accent}30`, cursor: 'pointer', letterSpacing: '0.06em' }}
                          className="bg">⊙ SCAN</span>
                      )}
                    </div>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead><tr>
                          {visibleTopoCols.map(col => (
                            <TH key={col.id} style={
                              col.id === 'individual_address' ? { width: 100, paddingLeft: 42 } :
                              col.id === 'device_type' ? { width: 90 } :
                              col.id === 'manufacturer' ? { width: 120 } :
                              col.id === 'model' ? { width: 110 } :
                              col.id === 'order_number' ? { width: 110 } :
                              col.id === 'serial_number' ? { width: 130 } :
                              col.id === 'status' ? { width: 110 } :
                              col.id === 'gas' ? { width: 50 } : {}
                        ...
                        <tbody>
                          {devs.map(d => (
                            <tr key={d.id} className="rh" style={{ borderLeft: '2px solid transparent' }}>
                              {tcv('individual_address') && <TD style={{ paddingLeft: 42 }}><PinAddr address={d.individual_address} wtype="device" style={{ color: C.accent, fontFamily: 'monospace' }} /></TD>}
                              {tcv('name') && <TD>{d.name}</TD>}
                              {tcv('device_type') && <TD><span style={{ color: C.muted }}>{d.device_type}</span></TD>}
                              {tcv('location') && spaces.length > 0 && <TD><SpacePath spaceId={d.space_id} spaces={spaces} style={{ color: C.dim, fontSize: 10 }} /></TD>}
                              {tcv('manufacturer') && <TD><PinAddr address={d.manufacturer} wtype="manufacturer" style={{ color: C.amber }}>{d.manufacturer || '—'}</PinAddr></TD>}
                              {tcv('model') && <TD><PinAddr address={d.model} wtype="model" style={{ color: C.amber, fontFamily: 'monospace', fontSize: 10 }}>{localizedModel(d) || '—'}</PinAddr></TD>}
                              {tcv('order_number') && <TD><span style={{ color: C.dim, fontFamily: 'monospace', fontSize: 10 }}>{d.order_number || '—'}</span></TD>}
                              {tcv('serial_number') && <TD><span style={{ color: C.dim, fontFamily: 'monospace', fontSize: 10 }}>{d.serial_number || '—'}</span></TD>}
                              {tcv('status') && <TD><Badge label={d.status.toUpperCase()} color={STATUS_COLOR[d.status] || C.dim} /></TD>}
                              {tcv('gas') && <TD><span style={{ color: C.dim }}>{(deviceGAMap[d.individual_address] || []).length}</span></TD>}
                            </tr>
                          ))}
                        </tbody>
                      </table>
      ...
      <div style={{ padding: '5px 14px', borderTop: `1px solid ${C.border}`, fontSize: 10, color: C.dim, display: 'flex', gap: 14 }}>
        {Object.entries(STATUS_COLOR).map(([s, c]) => (
          <span key={s} className="rh" onClick={() => setStatusFilter(p => p === s ? 'all' : s)}
            style={{ cursor: 'pointer', color: statusFilter === s ? c : C.dim, fontWeight: statusFilter === s ? 600 : 400 }}>
