...
export function GAPinPanel({ C, COLMAP, ga, linkedDevices, busConnected, gaTelegrams, gaMap, devMap, spaces, allCOs, onWrite, activeProjectId, onUpdateGA, onGroupJump }) {
  ...
  const [gaTab, setGaTab] = useState(() => localStorage.getItem('knx-pin-tab-ga') || 'overview');
  const handleGaTab = (t) => { setGaTab(t); localStorage.setItem('knx-pin-tab-ga', t); };
  useEffect(() => { setEditing(false); }, [ga.address]);
  ...
  const handleSend = async (val) => {
    ...
    setSending(true);
    try { await onWrite(ga.address, String(v), writeDpt); if (val === undefined) setWriteVal(''); }
    catch (_) {}
    setSending(false);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try { await onUpdateGA(ga.id, { name: editName.trim(), dpt: editDpt }); setEditing(false); }
    ...
    <div style={{ padding: 24 }}>
      <div style={{ width: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          <span style={{ color: C.purple }}><IconGroupAddr size={26} /></span>
          <div style={{ flex: 1 }}>
            <div onClick={pin ? () => pin('ga', ga.address) : undefined} style={{ fontFamily: "'DM Mono',monospace", fontWeight: 700, fontSize: 20, color: C.text, cursor: pin ? 'pointer' : 'default' }}>{ga.address}</div>
            {editing ? (
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', marginTop: 4 }}>
                ...
                <input value={editDpt} onChange={e => setEditDpt(e.target.value)} placeholder="DPT (e.g. 1.001)"
                  onKeyDown={e => { if (e.key === 'Enter') handleSave(); if (e.key === 'Escape') setEditing(false); }}
                  style={{ background: C.inputBg, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '4px 8px', color: C.text, fontSize: 11, fontFamily: 'monospace', width: 120 }} />
              ...
              <div onClick={onUpdateGA ? () => { setEditName(ga.name); setEditDpt(ga.dpt || ''); setEditing(true); } : undefined}
                style={{ fontSize: 12, color: C.muted, marginTop: 2, cursor: onUpdateGA ? 'pointer' : 'default' }}
                title={onUpdateGA ? 'Click to edit' : undefined}>{ga.name}</div>
            )}
          </div>
          {ga.dpt && <span title={dpt.hover(ga.dpt)}><Badge label={dpt.display(ga.dpt)} color={C.purple} /></span>}
        </div>
        {/* Tab bar */}
        <TabBar C={C} active={gaTab} onChange={handleGaTab} tabs={[
        ...
        {gaTab === 'overview' && <>
          ...
          <EditableRtfField label="COMMENT" value={ga.comment || ''} C={C}
            onSave={onUpdateGA ? (v) => onUpdateGA(ga.id, { comment: v }) : null} />
          {linkedDevices.length > 0 && (
            <GANetworkDiagram ga={ga} linkedDevices={linkedDevices} allCOs={allCOs} C={C} gaTelegrams={gaTelegrams} />
          )}
          {busConnected && (
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: '12px 16px', marginBottom: 20 }}>
              <div style={{ fontSize: 10, color: C.dim, letterSpacing: '0.08em', marginBottom: 10 }}>SEND TELEGRAM</div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                <select value={writeDpt} onChange={e => setWriteDpt(e.target.value)}
                  style={{ background: C.inputBg, border: `1px solid ${C.border}`, borderRadius: 4, padding: '6px 10px', color: C.text, fontSize: 11, fontFamily: 'inherit' }}>
                  {[['1','DPT 1 — Bool'],['2','DPT 2 — Bool+C'],['5','DPT 5 — 0–255'],['9','DPT 9 — Float'],['14','DPT 14 — Float32']].map(([v,l]) => (
                    <option key={v} value={v}>{l}</option>
                  ))}
                </select>
                {writeDpt === '1' ? (
                  <div style={{ display: 'flex', gap: 6 }}>
                    <Btn onClick={() => handleSend('1')} color={C.green} disabled={sending}>On</Btn>
                    <Btn onClick={() => handleSend('0')} color={C.red} disabled={sending}>Off</Btn>
                    <Btn onClick={() => handleSend(writeVal === '1' ? '0' : '1')} disabled={sending}>Toggle</Btn>
                  </div>
                ) : (
                  <>
                    ...
                    <Btn onClick={() => handleSend()} disabled={sending || writeVal === ''} color={C.accent}>
