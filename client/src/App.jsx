...
const VIEWS = [
  ...
  {
    id: 'devices',
    Icon: ({ size }) => <DeviceTypeIcon type="generic" size={size} />,
    label: 'Devices',
  },
...
export default function App() {
  ...
  const [dptMode, setDptMode] = useState(
    () => localStorage.getItem('knx-dpt-mode') || 'numeric',
  );
  const handleDptModeChange = (m) => {
    setDptMode(m);
  ...
  useEffect(() => {
    localStorage.setItem('knx-sidebar-width', sidebarWidth);
  }, [sidebarWidth]);
              ...
              >
                ...
                >
                  ...
                  {state.projectData && (
                    <>
                      <span>
                        {state.projectData.devices?.length ?? 0} devices
                      </span>
                      <span>·</span>
                      <span>
                        {state.projectData.gas?.length ?? 0} group addresses
                      </span>
                      <span>·</span>
                      <span>
                        {state.projectData.comObjects?.length ?? 0} group
                        objects
                      </span>
                    </>
                  )}
